...
@if($viewType == 'default')
    ...
    <div class="message-card" data-id="{{ $id }}">
        <p>{!! ($message == null && $attachment != null && @$attachment[2] != 'file') ? $attachment[1] : nl2br($message) !!}
            ...
            <a href="/image/{{$attachment[0]}}" style="color: #595959;" class="file-download" download>
    ...
    <div>
        <div class="message-card">
            <div class="image-file chat-image" style="width: 250px; height: 150px;background-image: url('{{ asset('/image'.'/'.$attachment[0]) }}')">
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="message-card mc-sender" data-id="{{ $id }}">
        <p>{!! ($message == null && $attachment != null && @$attachment[2] != 'file') ? $attachment[1] : nl2br($message) !!}
            <sub title="{{ $fullTime }}" class="message-time">
                ...
                {{-- If attachment is a file --}}
            @if(@$attachment[2] == 'file')
            <a href="/image/{{$attachment[0]}}" class="file-download" download>
                <span class="fas fa-file"></span> {{$attachment[1]}}</a>
            @endif
        </p>
    </div>
