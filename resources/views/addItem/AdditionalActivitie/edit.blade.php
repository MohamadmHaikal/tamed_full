...
@section('content')
    ...
    <div class="layout-px-spacing">
        <div class="col-lg-12 layout-spacing">
            <form method="post" action="{{ route('additionalactivitie.update', [$activitie->id]) }}" role="form">
                @csrf
                <div class="statbox widget box box-shadow mb-4">
                    ...
                    <div class="widget-content widget-content-area ">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" value="{{ $activitie->name }}" name="name">
                        </div>
                        <div class="form-group">
                            <label>{{ __('backend.main activity') }}</label>
                            <select class="form-control form-control-sm" name="main_id">
                                @foreach ($mainActivitys as $main)
                                    @if ($main->id == $activitie->activitie_id)
                                        <option value="{{ $main->id }}" selected>{{ $main->activity }}</option>
                                    @else
