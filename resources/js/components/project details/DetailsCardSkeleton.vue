<template>
    <div>
        <div class="about-area pb-70">
            <div class="containp1  container">
                <div class="mt-2" style="text-align: -webkit-center;">
                    <div class="col-md-12 col-sm-12 mt-3 text-center">
                        <div class="text-end">
                            <h3 class="title mt-5 title-skeleton mb-3" style="width: 40%; color:rgba(0, 0, 0, 0.0);">
                                صفقة لبيع مواد بناء</h3>
                            <span class="text-end title-skeleton "
                                style="color: rgb(205 205 205 / 47%);    font-size: 14px;"> الرقم المرجعي:</span>
                            <span class="text-end title-skeleton"
                                style="color: rgb(87 83 77) ;    font-size: 14px; padding-right: 5px;color:rgba(0, 0, 0, 0.0);">
                                25896374</span>
                            <div class="row mt-3">
                                <div class="col-md-9 col-sm-12 ">
                                    <span class="title-skeleton" style="color:rgb(205, 205, 205 ,0.0);">
                                        <i class="fas fa-star pr-2"
                                            style="color:rgb(205 205 205 / 47%) ; font-size: 17px;padding-left: 10px;">
                                        </i>0
                                        <span style="color:rgb(205, 205, 205 ,0.0)">(لايوجد تقييمات)</span>
                                    </span>
                                    <span
                                        style="margin-bottom: 2px; margin-right: 12px; height: 5px; width: 5px; background-color: #bbb; border-radius: 50%; display: inline-block;"></span>

                                    <span class="title-skeleton" style=" margin-right: 6px;">
                                        <i class="fas fa-map-marker-alt	"
                                            style="color:  rgba(205, 205, 205, 0.47);font-size: 20px; "></i>

                                        <span style="color: rgba(205, 205, 205, 0.0);"> الرياض - حي اليرموك</span>
                                    </span>
                                    <span
                                        style="margin-bottom: 2px; margin-right: 12px;height: 5px; width: 5px; background-color: #bbb; border-radius: 50%; display: inline-block;"></span>

                                    <span class="date-skeleton"
                                        style=" margin-right: 8px; color: rgba(205, 205, 205, 0.0);">
                                        <i class="far fa-clock project-header"
                                            style="color:  rgba(205, 205, 205, 0.47)"></i>
                                        التاريخ : 2022/02/22</span>
                                    <span
                                        style="margin-bottom: 2px; margin-right: 12px; height: 5px;width: 5px; background-color: #bbb; border-radius: 50%; display: inline-block;"></span>
                                    <span class="date-skeleton"
                                        style="margin-right: 5px; color:rgba(205, 205, 205, 0.0)">
                                        <i class="far fa-eye"
                                            style="color:  rgba(205, 205, 205, 0.47);font-size: 20px;margin-right: 5px;"></i>
                                        2655</span>

                                </div>
                                <div class="col-md-3 col-sm-12 text-center fav-share">

                                    <span class="date-skeleton"
                                        style=" margin-right: 2px; margin-bottom:2px ;color: rgba(205, 205, 205, 0.0);">
                                        <i class="far fa-heart"
                                            style="font-size:20px ;color: rgba(205, 205, 205, 0.47); margin-right: 5px;"></i>
                                        المفضلة</span>


                                    <span class="date-skeleton"
                                        style=" margin-right: 2px; margin-bottom:2px ; color: rgba(205, 205, 205, 0.0);">
                                        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                            viewBox="0 0 512 512"
                                            style="enable-background:new 0 0 512 512; width: 20px;margin-right: 15px; filter: invert(64%) sepia(0%) saturate(2476%) hue-rotate(86deg) brightness(118%) contrast(119%);"
                                            xml:space="preserve">
                                            <g>
                                                <polygon
                                                    points="334.9,120.6 256,41.7 177.1,120.6 156.3,99.7 256,0 355.7,99.7 	" />
                                                <rect x="241.1" y="20.8" width="29.8" height="312.6" />
                                                <path
                                                    d="M404.8,512H107.2c-25.3,0-44.7-19.4-44.7-44.7V199.4c0-25.3,19.3-44.7,44.7-44.7h104.2v29.8H107.2c-8.9,0-14.9,6-14.9,14.9 v267.9c0,8.9,6,14.9,14.9,14.9h297.7c8.9,0,14.9-6,14.9-14.9V199.4c0-8.9-6-14.9-14.9-14.9H300.7v-29.8h104.2 c25.3,0,44.7,19.3,44.7,44.7v267.9C449.5,492.6,430.1,512,404.8,512z" />
                                            </g>
                                        </svg>
                                        المشاركة</span>

                                </div>
                            </div>

                        </div>
                        <div class="text-center">
                            <div class="hh-gallery hh-grid-gallery mt-4"
                                style="position: relative; display: flex; Height: 30%;Overflow: hidden;">
                                <div class="controls">
                                    <a href="javascript: void(0);" class="view-gallery item-link"
                                        style="  margin-right: 5px;  background: #eee; border-radius: 3px; font-size: 13px;font-weight: 500;padding: 5px 10px;color: #333;display: flex;align-items: center;">
                                        <span style="color: #dedfe0;">عرض كل الصور</span>
                                        <i class="ti-gallery"></i>
                                    </a>
                                    <span><a href="javascript: void(0);" class="view-gallery item-link"
                                            style="  margin-right: 5px;  background: #eee; border-radius: 3px; font-size: 13px;font-weight: 500;padding: 5px 10px;color: #333;display: flex;align-items: center;">
                                            <span style="color: #dedfe0;">عرض كل الفيديوهات</span>
                                            <i class="ti-gallery"></i>
                                        </a></span>
                                </div>
                                <div class="item" style="flex: 2 1 0%; margin-right: 4px;">
                                    <div class="item-inner" style="width: 100%;padding-top: 130%;position: relative;">
                                        <div class="img-skeleton"
                                            style=" border-radius: 1px 18px 18px 1px;position: absolute;top: 0;left: 0;width: 100%;height: 100%;object-fit: cover;">
                                        </div>
                                    </div>
                                </div>
                                <div class="item" style="flex: 2 1 0%;margin-right: 4px;">
                                    <div class="item-inner" style="width: 100%;padding-top: 130%;position: relative;">
                                        <div class="img-skeleton"
                                            style="position: absolute;top: 0;  left: 0; width: 100%; height: 100%;  object-fit: cover;">
                                        </div>
                                    </div>
                                </div>
                                <div class="item item-small" style="margin-right: 4px;flex: 1 1 0%;">
                                    <div class="item-outer" style="height: calc(50% - 2px);">
                                        <div class="item-inner"
                                            style="height: 100%; padding-top: 0;padding-top: 130%; position: relative;">
                                            <div class="img-skeleton"
                                                style="position: absolute; top: 0;left: 0;width: 100%;height: 100%;object-fit: cover;">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="space" style=" margin-top: 4px;">
                                    </div>
                                    <div class="item-outer" style="height: calc(50% - 2px);">
                                        <div class="item-inner"
                                            style="height: 100%;width: 100%;position: relative; padding-top: 0;">
                                            <div class="img-skeleton"
                                                style="position: absolute;top: 0;left: 0; width: 100%;height: 100%;object-fit: cover;">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="item" style="flex: 2 1 0%;margin-right: 4px;">
                                    <div class="item-inner" style="width: 100%;padding-top: 130%;position: relative;">
                                        <div class="img-skeleton"
                                            style=" border-radius: 18px 1px 1px 18px;position: absolute;top: 0px;left: 0px;width: 100%; height: 100%;object-fit: cover;">
                                        </div>
                                    </div>
                                </div>

                            </div>
                        </div>
                        <div class="row" style="padding-right: 0px; padding-left: 0px;">
                            <div class="col-md-7  col-sm-12 text-end mt-3">
                                <div style="border-bottom: 1px solid rgb(238, 239, 241); padding-bottom: 10%;">

                                    <h6 class="des-skeleton" style="color:#fff ;margin-top: 5%; font-weight: 700;">الوصف
                                    </h6>
                                    <p class="title-skeleton"
                                        style="max-width: 52ch; color: #ffffff00;font-weight: 400;">بيع مواد حديد
                                        بناء بالرياض حديد سابك شبه جديد ولا يوجد به اي
                                        عيوب الكمية 200 طن حديد</p>
                                </div>
                                <div class="row">
                                    <div class="col-md-8  project-deadline">
                                        <p class="date-skeleton"
                                            style=" padding-right: 2%;margin-top: 5%;width:47%;font-weight: 500; font-size: 13px;color: #fff;">
                                            اخر موعد لتقديم العروض </p>

                                        <span class="title-skeleton"
                                            style="color:#ffffff00;font-size:15px;padding-right:2px;">
                                            <i class="far fa-calendar-alt"
                                                style="color:#fff;padding-right: 2%;   font-size: 15px; "></i>
                                            2022-05-22
                                        </span>
                                        <span class="title-skeleton"
                                            style="color:#ffffff00;font-size:15px;padding-right:5px;">
                                            <i class="far fa-clock" style="color:#fff;    font-size: 15px;"></i>
                                            12:00</span>
                                    </div>
                                    <div class="col-md-4 project-activite">
                                        <span class="title-skeleton" style="color: #fff;    font-size: 14px;">
                                            النشاط:</span>
                                        <span class="title-skeleton"
                                            style="color: #ffffff00 ;    font-size: 14px; padding-right: 5px;">
                                            مقاولات</span>

                                    </div>
                                    <div class="col-md-12 mt-4">
                                        <div class="tab title-skeleton" style="border-color:#f6f7f8 ;">
                                            <button id="buttonlinks" class="tablinks" style="background-color:#eeeeee ;"
                                                @click="openCity($event, 'London')">الميزات
                                                المقدمة</button>
                                            <button class="tablinks" style="background: transparent; color: #fff;">ملفات
                                                الصفقة</button>
                                            <button class="tablinks" style="background: transparent;color: #fff;">
                                                شروط الشراء</button>
                                        </div>

                                        <div id="London" class="tabcontent">
                                            <h3 class="date-skeleton"
                                                style="color:#fff; display: block;    width: 50%;">المواصفات والميزات
                                            </h3>
                                            <div class="row mt-5"
                                                style=" border-bottom: 1px solid rgb(238, 239, 241);padding-bottom: 3%;">
                                                <div class="row"
                                                    style=" border-bottom: 1px solid rgb(238, 239, 241);padding-bottom: 3%;">

                                                    <div class="col-md-4 col-6 conditional ref"
                                                        style="padding-right:0px ; padding-left:0px ;"><svg
                                                            xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                                            width="30" height="64" viewBox="0 0 64 64"
                                                            style=" fill:#fff;">
                                                            <path
                                                                d="M 32 3 L 1 28 L 1.4921875 28.654297 C 2.8591875 30.477297 5.4694688 30.791703 7.2304688 29.345703 L 32 9 L 56.769531 29.345703 C 58.530531 30.791703 61.140812 30.477297 62.507812 28.654297 L 63 28 L 54 20.742188 L 54 8 L 45 8 L 45 13.484375 L 32 3 z M 32 13 L 8 32 L 8 56 L 56 56 L 56 35 L 32 13 z M 26 34 L 38 34 L 38 52 L 26 52 L 26 34 z">
                                                            </path>
                                                        </svg><span
                                                            style="color:#c2c2c200 ; padding-right: 5px; padding-left:0px ;">هل
                                                            يوجد
                                                            سكن</span> <span
                                                            style="margin-bottom: 2px;      margin-left: 15px; margin-right: 20px; height: 7px; width: 7px; background-color: #fff; border-radius: 50%; display: inline-block;"></span>
                                                        <br><i class="fa fa-check text-center"
                                                            style="font-size: 24px;margin-right: 65px;color: #fff;"></i>
                                                    </div>

                                                    <div class="col-md-4 col-6 conditional ref"
                                                        style="padding-right:0px ; padding-left:0px ;"><i
                                                            class="fa fa-house-damage"
                                                            style="    font-size: 25px;  color: #fff; margin-top: 15px;"></i>
                                                        <span style="color:#c2c2c200 ; padding-right: 5px;">هل تتوفر
                                                            كهرباء</span> <span
                                                            style="margin-bottom: 2px;      margin-right: 10px; height: 7px; width: 7px; background-color: #fff; border-radius: 50%; display: inline-block;"></span>
                                                        <br><i class="fa fa-check text-center"
                                                            style="font-size: 24px;margin-right: 65px;color: #fff;     margin-top: 20px;"></i>
                                                    </div>

                                                    <div class="col-md-4 col-6 conditional ref"
                                                        style="padding-right:0px ;margin-right:0px;">
                                                        <i class="fa fa-bus"
                                                            style="    font-size: 25px;  color: #fff; margin-top: 15px;"></i>
                                                        <span style="color:#c2c2c200; padding-right: 5px;">هل تتوفر
                                                            مواصلات</span>
                                                        <br><i class="fa fa-check text-center"
                                                            style="font-size: 24px;margin-right: 65px;color: #fff;     margin-top: 20px;"></i>
                                                    </div>

                                                </div>
                                                <div class="col-md-4 col-6 conditional ref mt-3"
                                                    style="padding-right:10px ; margin-right:2px;"><i
                                                        class="fa fa-money-bill-wave"
                                                        style="    font-size: 25px;  color: #fff; margin-top: 15px;"></i>
                                                    <span style="color:#c2c2c200 ; padding-right: 5px;">
                                                        دفعة مقدمة</span>
                                                    <br><i class="fa fa-check text-center"
                                                        style="font-size: 24px;margin-right: 65px;color: #fff;     margin-top: 20px;"></i>
                                                </div>

                                            </div>
                                        </div>

                                        <div id="Paris" class="tabcontent">
                                            <h3 style="color:#000000">تحميل ملفات الصفقة</h3>
                                            <table class="mt-5"
                                                style=" border-collapse: separate;border-spacing: 0;min-width: 350px;">

                                                <tr>
                                                    <td
                                                        style=" border-right: 1px solid #ccc; border-top: 1px solid #ccc; border-top-right-radius: 20px;">
                                                        <div style="padding: 30px;"> <img
                                                                src="../../assets/images/pdf.png" alt=""
                                                                style=" width: 30%;"> <span
                                                                style=" color:#019aa2;  padding-right: 2px;  font-size: 20px;">
                                                                <a href="javascript:void(0);"
                                                                    style=" color:#019aa2;">تحميل</a></span>
                                                        </div>
                                                    </td>
                                                    <td
                                                        style=" border-top: 1px solid #ccc;    border-left: 1px solid #ccc; border-right: 1px solid #ccc; border-top-left-radius: 20px;">
                                                        <div style="padding: 30px;"><img
                                                                src="../../assets/images/3d.png" alt=""
                                                                style=" width: 30%;"> <span
                                                                style=" color:#019aa2;  padding-right: 2px;  font-size: 20px;">
                                                                <a href="javascript:void(0);"
                                                                    style=" color:#019aa2;">تحميل</a></span>
                                                        </div>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td
                                                        style="border-right: 1px solid #ccc;     border-bottom: 1px solid #ccc;   border-top: 1px solid #ccc; border-bottom-right-radius: 20px;">
                                                        <div style="padding: 30px;"><img
                                                                src="../../assets/images/Excel.png" alt=""
                                                                style=" width: 30%;"> <span
                                                                style=" color:#019aa2; padding-right: 2px;   font-size: 20px;">
                                                                <a href="javascript:void(0);"
                                                                    style=" color:#019aa2;">تحميل</a></span>
                                                        </div>
                                                    </td>
                                                    <td style="border: 1px solid #ccc;border-bottom-left-radius: 20px;">
                                                        <div style="padding: 30px;"><img
                                                                src="../../assets/images/autoca.png" alt=""
                                                                style=" width: 30%;"> <span
                                                                style=" color:#019aa2;   padding-right: 2px; font-size: 20px;">
                                                                <a href="javascript:void(0);"
                                                                    style=" color:#019aa2;">تحميل</a></span>
                                                        </div>
                                                    </td>
                                                </tr>
                                            </table>
                                        </div>

                                        <div id="Tokyo" class="tabcontent">
                                            <h3 style="color:#000000">شروط شراء الصفقة</h3>
                                            <div class="mt-5"
                                                style="    border: 1px solid #ccc;border-radius: 25px;padding: 20px;">
                                                <div class="row mt-4">
                                                    <div class="col-md-4 col-6  text-center">
                                                        <img src="../../assets/images/amala.png" alt=""
                                                            style="width:45px;">
                                                        <br>
                                                        <small style="color:#333333 ;">العمالة</small>
                                                        <br>
                                                        <small style="color:#019aa2 ;">على الكفالة او عقد اجير</small>
                                                    </div>

                                                    <div class="col-md-4 col-6 text-center">
                                                        <img src="../../assets/images/tasnef.png" alt=""
                                                            style="width:40px;">
                                                        <br>
                                                        <small style="color:#333333 ;">فئة التصنيف</small>
                                                        <br>
                                                        <small style="color:#019aa2 ;">لايهم</small>
                                                    </div>
                                                    <div class="col-md-4 col-6 text-center">
                                                        <img src="../../assets/images/daman.png" alt=""
                                                            style="width:50px;">
                                                        <br>
                                                        <small style="color:#333333 ;">ضمان بنكي</small>
                                                        <br>
                                                        <small style="color:#019aa2 ;">لايهم</small>
                                                    </div>
                                                </div>
                                            </div>
                                            <small
                                                style="position: relative; top: 26px;right: 40px; background-color: #ffffff;padding: 0px 10px 0px 10px;">الاوراق
                                                المطلوبة</small>
                                            <div class="mt-3"
                                                style="border: 1px solid #ccc;border-radius: 25px;padding: 20px;">
                                                <div class="row">
                                                    <div class="col-md-4  col-6 text-center">
                                                        <a href="javascript:void(0);" class="btn paper"> صورة السجل
                                                            التجاري</a>
                                                    </div>

                                                    <div class="col-md-4 col-6 text-center">
                                                        <a href="javascript:void(0);" class="btn paper">الدخل
                                                            والزكاة</a>
                                                    </div>
                                                    <div class="col-md-4 col-6 text-center">
                                                        <a href="javascript:void(0);" class="btn paper"> ضريبة القيمة
                                                            المضافة </a>
                                                    </div>
                                                    <div class="col-md-4 col-6 text-center">
                                                        <a href="javascript:void(0);" class="btn paper"> شهادة تصنيف
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                </div>

                            </div>
                            <div class="col-md-5 col-sm-12 price-card">
                                <div class="card-skeleton"
                                    style="width: 60%; height: 330px; margin-top: 6%; border-radius: 12px; margin-bottom: 5%;  box-shadow: rgb(236 233 233) 1px 1px 5px 2px;">
                                    <div style="border-bottom: 1px solid #eeeff1 ;">
                                        <h5 style="padding-top: 8%;text-align: right;padding-right: 10%;color: #fff;">
                                            قيمة الصفقة
                                        </h5>
                                        <h6 style="color: #FFFFFF00; text-align: right;padding-right: 15%;"> 600,000
                                            ريال
                                        </h6>

                                    </div>
                                    <div style="text-align: right;">
                                        <p
                                            style=" color:#fff; text-align: right; padding-right: 13%;padding-top: 10%;font-weight: 500; font-size: 13px;">
                                            اخر موعد لتقديم العروض </p>
                                        <i class="far fa-calendar-alt"
                                            style="color:#fff;padding-right: 13%;   font-size: 15px; "></i>
                                        <span style="color:#FFFFFF00;font-size:12px;padding-right:2px;"> 2022-05-22
                                        </span>
                                        <span style="color:#FFFFFF00;font-size:12px;padding-right:5px;">
                                            <i class="far fa-clock" style="color:#fff;    font-size: 15px;"></i>
                                            12:00</span>
                                        <div style="text-align:center ;">
                                            <router-link to="/SubmitQuotes" class="btn btn-logo"
                                                style="border-radius:16px;background-color:rgb(238 238 238);; width:85%;color:#ffffff61;margin-top:25px;padding:7px 30px 7px 30px;">
                                                تقديم
                                                شراء للصفقة </router-link>

                                        </div>
                                        <div class="text-center mt-5">
                                            <span class="text-end " style="color: #fff;    font-size: 12px;"> الرقم
                                                المرجعي للصفقة:</span>
                                            <span class="text-end"
                                                style="color: #FFFFFF00 ;font-size: 12px; padding-right: 5px;">
                                                25896374</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="card-skeleton"
                                    style="width: 60%;  margin-top: 6%; border-radius: 12px;  box-shadow: rgb(236 233 233) 1px 1px 5px 2px;     margin-bottom: 25px;">
                                    <div style="border-bottom: 1px solid #eeeff1 ;">
                                        <h5 style="padding-top: 8%;text-align: center;  color: #fff;  font-size: 95%;">
                                            معلومات صاحب
                                            الطلب
                                        </h5>


                                    </div>
                                    <div class="mt-3" style="text-align: center;  padding-bottom: 15px; ">
                                        <span class="text-end " style="color: #fff;    font-size: 14px;"> مقدم
                                            الطلب:</span>
                                        <span class="text-end"
                                            style="color: #FFFFFF00 ;    font-size: 14px; padding-right: 5px;"> شركة
                                            فرسان للمقاولات</span>


                                        <div style="text-align:center ;">
                                            <router-link to="/profile" class="btn btn-logo"
                                                style=" border-radius: 9px; background-color: #eeeeee;   width: 69%;   color: white; font-size: 90%; margin-top: 25px;  padding: 1px 23px;">
                                                الملف الشخصي للمنشآة </router-link>

                                        </div>
                                        <div class="mt-3">

                                            <span class="far fa-star " style=" color:#eeeeee ;   font-size: 20px;"
                                                aria-hidden></span>
                                            <span class="far fa-star "
                                                style="  color:#eeeeee ;  font-size: 20px;"></span>
                                            <span class="fas fa-star checked"
                                                style="  color:#eeeeee ;    font-size: 20px;"></span>
                                            <span class="fas fa-star checked"
                                                style="  color:#eeeeee ;    font-size: 20px;"></span>
                                            <span class="fas fa-star checked"
                                                style="  color:#eeeeee ;    font-size: 20px;"></span>
                                        </div>
                                        <div class="row text-center mt-2">
                                            <div class="col-md-5 col-sm-2"
                                                style="text-align: center;margin-bottom: 12px;">
                                                <a href="javascript:void(0);" style="color: #fff;">
                                                    <p style="position: relative; top: 18px;color:#fff">رسالة</p>
                                                    <i class="far fa-comment-alt mb-2"
                                                        style="font-size: 25px; color: #fff;"></i>
                                                </a>

                                            </div>
                                            <div class="col-md-7 col-sm-10">
                                                <a href="javascript:void(0);" class="btn report"
                                                    style="border-color: #fff; color: #fff;">
                                                    تقديم بلاغ على الصفقة </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                        </div>


                    </div>
                </div>


            </div>
        </div>


    </div>
</template>

<script>

export default {
    methods: {

        openCity(evt, cityName) {
            var i, tabcontent, tablinks;
            tabcontent = document.getElementsByClassName("tabcontent");
            for (i = 0; i < tabcontent.length; i++) {
                tabcontent[i].style.display = "none";
            }
            tablinks = document.getElementsByClassName("tablinks");
            for (i = 0; i < tablinks.length; i++) {
                tablinks[i].className = tablinks[i].className.replace(" active", "");
            }
            document.getElementById(cityName).style.display = "block";

            evt.currentTarget.className += " active";
        },
    },
    mounted: function () {
        var element = document.getElementById("buttonlinks");
        element.classList.add("active");
        document.getElementById('London').style.display = "block";
    }
}


</script>
<style>
/* Style the tab */
.tab {
    overflow: hidden;
    border: 1px solid #ccc;

    border-radius: 25px;
    padding-left: 10px;
    padding-right: 10px;
    padding-top: 2px;
}

/* Style the buttons inside the tab */



/* Create an active/current tablink class */

/* Style the tab content */
</style>
<style>
table {
    border-collapse: unset;
    width: 100%;
}
</style>
<style scoped>
.title-skeleton {
    padding: 0px 16px 0px 16px;
    background: #f6f7f8;
    background-image: linear-gradient(to right, #f6f7f8 0%, #edeef1 20%, #f6f7f8 40%, #f6f7f8 100%);
    background-repeat: no-repeat;
    background-size: 800px 280px;
    position: relative;
    -webkit-animation-duration: 1s;
    -webkit-animation-fill-mode: forwards;
    -webkit-animation-iteration-count: infinite;
    -webkit-animation-name: placeholderShimmer;
    -webkit-animation-timing-function: linear;

}

.card-skeleton {
    background: #f6f7f8;
    background-image: linear-gradient(to right, #f6f7f8 0%, #edeef1 20%, #f6f7f8 40%, #f6f7f8 100%);
    background-repeat: no-repeat;
    background-size: 800px 330px;
    position: relative;
    -webkit-animation-duration: 1s;
    -webkit-animation-fill-mode: forwards;
    -webkit-animation-iteration-count: infinite;
    -webkit-animation-name: placeholderShimmer;
    -webkit-animation-timing-function: linear;

}

.date-skeleton {
    padding: 0px 0px 0px 16px;
    background: #f6f7f8;
    background-image: linear-gradient(to right, #f6f7f8 0%, #edeef1 20%, #f6f7f8 40%, #f6f7f8 100%);
    background-repeat: no-repeat;
    background-size: 800px 280px;
    position: relative;
    -webkit-animation-duration: 1s;
    -webkit-animation-fill-mode: forwards;
    -webkit-animation-iteration-count: infinite;
    -webkit-animation-name: placeholderShimmer;
    -webkit-animation-timing-function: linear;

}

.ref {
    padding: 0px 16px 0px 16px;

    background: #f6f7f8;
    background-image: linear-gradient(to right, #f6f7f8 0%, #edeef1 20%, #f6f7f8 40%, #f6f7f8 100%);
    background-repeat: no-repeat;
    background-size: 800px 280px;
    display: inline-block;
    position: relative;
    -webkit-animation-duration: 1s;
    -webkit-animation-fill-mode: forwards;
    -webkit-animation-iteration-count: infinite;
    -webkit-animation-name: placeholderShimmer;
    -webkit-animation-timing-function: linear;
}

.des-skeleton {
    padding: 5px 16px 5px 16px;

    background: #f6f7f8;
    background-image: linear-gradient(to right, #f6f7f8 0%, #edeef1 20%, #f6f7f8 40%, #f6f7f8 100%);
    background-repeat: no-repeat;
    background-size: 800px 280px;
    display: inline-block;
    position: relative;
    -webkit-animation-duration: 1s;
    -webkit-animation-fill-mode: forwards;
    -webkit-animation-iteration-count: infinite;
    -webkit-animation-name: placeholderShimmer;
    -webkit-animation-timing-function: linear;
}

.img-skeleton {
    padding: 0px 16px 0px 16px;
    background: #f6f7f8;
    background-image: linear-gradient(to right, #f6f7f8 0%, #edeef1 20%, #f6f7f8 40%, #f6f7f8 100%);
    background-repeat: no-repeat;
    background-size: 800px 370px;
    display: inline-block;
    position: relative;
    -webkit-animation-duration: 1s;
    -webkit-animation-fill-mode: forwards;
    -webkit-animation-iteration-count: infinite;
    -webkit-animation-name: placeholderShimmer;
    -webkit-animation-timing-function: linear;

}


@-webkit-keyframes placeholderShimmer {
    0% {
        background-position: -468px 0;
    }

    100% {
        background-position: 468px 0;
    }
}
</style>