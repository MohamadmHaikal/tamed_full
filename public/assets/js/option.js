    ...
    var HHOptions = {
        ...
        initUpload: function (el) {
            ...
            el.on('click', '.hh-upload-wrapper .add-attachment', function () {
                console.log(123);
                let t = $(this);
                document.querySelector('#' + t.data('file')).addEventListener('change', function () {

                    if (this.files && this.files[0]) {
                        console.log(t.data('img'));
                        $('.hh-upload-wrapper').find('#' + t.data('img')).attr('src', URL.createObjectURL(this.files[0]));
                        var file = $('#' + t.data('file')).val();
                        console.log(file);
                        var fd = new FormData();
                        fd.append('file', $('#' + t.data('file')).get(0).files[0]);
                        //  $("#laravel-ajax-file-upload").submit();

                        //  document.getElementById("laravel-ajax-file-upload").submit();
                        //console.log( document.getElementById("laravel-ajax-file-upload"));

                        console.log(12121525555555555555555555);
                        $.ajaxSetup({
                            headers: {
                        ...
                        // var formData = new FormData(document.querySelector('#form-logo'));
                        $.ajax({
                            type: 'POST',
                            url: "store-file",
                            data: fd,
                            ...
                            success: (data) => {
                                ...
                                document.getElementById(t.data('inp')).value = data['file'];
                                console.log(data);
                            },
                            error: function (data) {
                                console.log(data);
        ...
        mediaRender: function (el, multi, target = null) {
            ...
            $('body').append('<div id="hh-media-frame-modal" class="modal fade ' + _class + '" tabindex="-1" role="dialog" aria-hidden="true">\n' +
                ...
                '                            </a></h4>\n' +
                '                    <form class="form form-sm form-search-media" action="" method="post"><input type="text" class="form-control" name="search" placeholder="Search ..."></form>' +
                '                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×\n' +
                '                    </button>\n' +
                '                </div>\n' +
                '                <div class="modal-body">' +
                '                   <div id="hh-media-add-new" class="hh-media-upload-area collapse mt-3">\n' +
                '                            <form action="" method="post" class="hh-dropzone"\n' +
