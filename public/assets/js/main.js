(function($) {
    ...
    $(document).ready(function(){
        ...
        var selectedMenuCategories = $('.sidebar-wrapper .menu-categories li a.app-submenu.active');
        var selectedSubMenu = $('.sidebar-wrapper .sidebar-submenu li a.app-submenu.active');

        if (selectedMenuCategories) {
            $('.sidebar-wrapper .menu-categories li a.dropdown-toggle.app-submenu.active').attr("data-active", true);
        }

        if (selectedSubMenu) {
            $(selectedSubMenu).closest('li').addClass("active");
            $(selectedSubMenu).closest('ul').addClass("show");

            //find tile menu
            var subMenuEle = $(selectedSubMenu).closest('div.submenu');
            var subMenuId = $(subMenuEle).attr("id");
            var tileMenuId = subMenuId.replace("Menu", "");
            $('.sidebar-wrapper .menu-categories li.menu').find("#" + tileMenuId).attr("data-active", true);

            var toogleDropdown = $(selectedSubMenu).parent().parent().siblings('a').first();
            if (toogleDropdown) {
                $(toogleDropdown).attr("aria-expanded", true);
            }
        }
    });
    var App = function() {
